...
PrintfClass& PrintfClass::sa(char* stackStartPtr,char c){
   ...
   SPRStackArrayConcrete<char,2> buffer;
          int spr_intScratcha9;spr_intScratcha9=0;chck_accs0(buffer,spr_intScratcha9,171)
   buffer._array[spr_intScratcha9]=c;
          int spr_intScratchab;spr_intScratchab=1;chck_accs0(buffer,spr_intScratchab,173)
   buffer._array[spr_intScratchab]='\0';
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
PrintfClass& PrintfClass::sa(char* stackStartPtr,String_16& str){
   ...
   if(bufferDyn._theObject->_sz<=str.length(stackStartPtr)){
      ...
      int spr_intScratchad;
      spr_intScratchad=newSize;
      if(spr_intScratchad<0)throw "invalid size requested for heap allocation";
      bufferDyn=new(spr_intScratchad) SPRArray<char>(spr_intScratchad)
      ;
   ...
   while(i<str.length(stackStartPtr)){
             int spr_intScratchae;spr_intScratchae=i;chck_accs_p0(bufferDyn,spr_intScratchae,176)
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      bufferDyn._theObject->_array[spr_intScratchae]=str.getAt(stackStartPtr,i);
      i++;
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
          ...
          int spr_intScratchb0;spr_intScratchb0=i;chck_accs_p0(bufferDyn,spr_intScratchb0,178)
   bufferDyn._theObject->_array[spr_intScratchb0]='\0';
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
void ThreadLocalStorage::getEntry(char* stackStartPtr,int id,SPRSmartPtr<ThreadLocalStorageEntry>& ret){
   ...
   SPRSmartPtr<ThreadLocalStorageList> list;list=0U;
          ...
          int spr_intScratchb2;spr_intScratchb2=id;chck_accs0(list._theObject->_entries,spr_intScratchb2,180)
   ret=list._theObject->_entries._array[spr_intScratchb2];
...
void ThreadLocalStorage::setEntry(char* stackStartPtr,int id,SPRSmartPtr<ThreadLocalStorageEntry> entry){
   ...
   SPRSmartPtr<ThreadLocalStorageList> list;list=0U;
          ...
          int spr_intScratchb4;spr_intScratchb4=id;chck_accs0(list._theObject->_entries,spr_intScratchb4,182)
   list._theObject->_entries._array[spr_intScratchb4]=entry;
...
int SPRFile::readChar(char* stackStartPtr,char& out){
   char SPR_stack_dummy_var;
   if(_readBuffer==0U){
      int spr_intScratchb6;
      spr_intScratchb6=16000;
      if(spr_intScratchb6<0)throw "invalid size requested for heap allocation";
      _readBuffer=new(spr_intScratchb6) SPRArray<char>(spr_intScratchb6)
      ;
      _validBytes=0;
      _readPtr=0;
      _fpos=0;
   ...
   if(_validBytes<=_readPtr){
      ...
      _fpos=_fpos+_validBytes;
          ...
          int spr_intScratchb7;spr_intScratchb7=_readPtr;chck_accs_p0(_readBuffer,spr_intScratchb7,185)
   out=_readBuffer._theObject->_array[spr_intScratchb7];
   _readPtr++;
   return 1;
...
void SPRFile::__test(char* stackStartPtr){
   char SPR_stack_dummy_var;
   PrintfClass pfc;
   SPRStackArrayConcrete<char,29> spr_StringScratchb9;strcpy(spr_StringScratchb9._array,"c:\\temp\\spr_test_file.txt");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   SPRFile f(stackStartPtr,spr_StringScratchb9,0);
   SPRStackArrayConcrete<char,12> spr_StringScratchba;strcpy(spr_StringScratchba._array,"0123456789");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   f.write(stackStartPtr,0,10,spr_StringScratchba);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if(f.size(stackStartPtr)!=10){
      SPRStackArrayConcrete<char,36> spr_StringScratchbb;strcpy(spr_StringScratchbb._array,"SPRFile::__test() step 1 failed$\n");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      SPRStackArrayConcrete<char,2> spr_StringScratchbc;strcpy(spr_StringScratchbc._array,"");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratchbb).sa(stackStartPtr,spr_StringScratchbc).pr(stackStartPtr);
      
   }
   SPRStackArrayConcrete<char,12> spr_StringScratchbd;strcpy(spr_StringScratchbd._array,"0123456789");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   f.write(stackStartPtr,10,10,spr_StringScratchbd);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if(f.size(stackStartPtr)!=20){
      SPRStackArrayConcrete<char,36> spr_StringScratchbe;strcpy(spr_StringScratchbe._array,"SPRFile::__test() step 2 failed$\n");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      SPRStackArrayConcrete<char,2> spr_StringScratchbf;strcpy(spr_StringScratchbf._array,"");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratchbe).sa(stackStartPtr,spr_StringScratchbf).pr(stackStartPtr);
      
   }
   {//begin of SPR for statement
   int i;i=0;
   while(i<1000){
      SPRStackArrayConcrete<char,22> spr_StringScratchc0;strcpy(spr_StringScratchc0._array,"abcdefghijklmnopqrst");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      f.write(stackStartPtr,i*20+20,20,spr_StringScratchc0);
      i++;
   ...
   if(f.size(stackStartPtr)!=20020){
      SPRStackArrayConcrete<char,36> spr_StringScratchc1;strcpy(spr_StringScratchc1._array,"SPRFile::__test() step 3 failed$\n");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      SPRStackArrayConcrete<char,2> spr_StringScratchc2;strcpy(spr_StringScratchc2._array,"");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratchc1).sa(stackStartPtr,spr_StringScratchc2).pr(stackStartPtr);
   ...
   while(i<10){
      ...
      char c;c=((char)i+((int)'0'));
             int spr_intScratchc3;spr_intScratchc3=0;chck_accs0(buf,spr_intScratchc3,197)
      if(buf._array[spr_intScratchc3]!=c){
         SPRStackArrayConcrete<char,36> spr_StringScratchc5;strcpy(spr_StringScratchc5._array,"SPRFile::__test() step 4 failed$\n");
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
         SPRStackArrayConcrete<char,2> spr_StringScratchc6;strcpy(spr_StringScratchc6._array,"");
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
         pfc.fstr(stackStartPtr,spr_StringScratchc5).sa(stackStartPtr,spr_StringScratchc6).pr(stackStartPtr);
   ...
   while(i<10){
      ...
      char c;c=((char)i+((int)'0'));
             int spr_intScratchc7;spr_intScratchc7=0;chck_accs0(buf,spr_intScratchc7,201)
      if(buf._array[spr_intScratchc7]!=c){
         SPRStackArrayConcrete<char,36> spr_StringScratchc9;strcpy(spr_StringScratchc9._array,"SPRFile::__test() step 5 failed$\n");
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   while(i<1000){
      ...
      f.read(stackStartPtr,i*20+20,20,buf);
             int spr_intScratchcb;spr_intScratchcb=20;chck_accs0(buf,spr_intScratchcb,205)
      buf._array[spr_intScratchcb]='\0';
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      String_16 str(stackStartPtr,buf);
      SPRStackArrayConcrete<char,22> spr_StringScratchcd;strcpy(spr_StringScratchcd._array,"abcdefghijklmnopqrst");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if(str.compare(stackStartPtr,spr_StringScratchcd)!=0){
         SPRStackArrayConcrete<char,36> spr_StringScratchce;strcpy(spr_StringScratchce._array,"SPRFile::__test() step 6 failed$\n");
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
int AlphabeticSorter::isBelow(char* stackStartPtr,SPRStackArray<char>& a,SPRStackArray<char>& b){
   ...
   while((i<a._sz)&&(i<b._sz)){
             int spr_intScratchd0;spr_intScratchd0=i;chck_accs0(a,spr_intScratchd0,210)
             int spr_intScratchd2;spr_intScratchd2=i;chck_accs0(b,spr_intScratchd2,212)
      if(a._array[spr_intScratchd0]!=b._array[spr_intScratchd2]){
                int spr_intScratchd4;spr_intScratchd4=i;chck_accs0(a,spr_intScratchd4,214)
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
                int spr_intScratchd6;spr_intScratchd6=i;chck_accs0(b,spr_intScratchd6,216)
         if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
         int delta;delta=this->ordinal(stackStartPtr,a._array[spr_intScratchd4])-this->ordinal(stackStartPtr,b._array[spr_intScratchd6]);
         if(delta>0){
            return 1;
...
void AlphabeticSorter::qsort(char* stackStartPtr,SPRStackArray<SPRSmartPtr<String_16> > wordlist,int lowerBound,int upperBound){
   ...
   SPRSmartPtr<String_16> pivot;
          int spr_intScratchd8;spr_intScratchd8=pivotIndex;chck_accs0(wordlist,spr_intScratchd8,218)
   pivot=wordlist._array[spr_intScratchd8];
   ...
   while(lower<=upper){
      ...
      if(lower<upper){
         SPRSmartPtr<String_16> speicher;
                int spr_intScratchda;spr_intScratchda=lower;chck_accs0(wordlist,spr_intScratchda,220)
         speicher=wordlist._array[spr_intScratchda];
                int spr_intScratchdc;spr_intScratchdc=lower;chck_accs0(wordlist,spr_intScratchdc,222)
                int spr_intScratchde;spr_intScratchde=upper;chck_accs0(wordlist,spr_intScratchde,224)
         wordlist._array[spr_intScratchdc]=wordlist._array[spr_intScratchde];
                int spr_intScratche0;spr_intScratche0=upper;chck_accs0(wordlist,spr_intScratche0,226)
         wordlist._array[spr_intScratche0]=speicher;
         
      }
      
   };
   SPRSmartPtr<String_16> temp;
          int spr_intScratche2;spr_intScratche2=upperBound;chck_accs0(wordlist,spr_intScratche2,228)
   temp=wordlist._array[spr_intScratche2];
          int spr_intScratche4;spr_intScratche4=upperBound;chck_accs0(wordlist,spr_intScratche4,230)
          int spr_intScratche6;spr_intScratche6=lower;chck_accs0(wordlist,spr_intScratche6,232)
   wordlist._array[spr_intScratche4]=wordlist._array[spr_intScratche6];
          int spr_intScratche8;spr_intScratche8=lower;chck_accs0(wordlist,spr_intScratche8,234)
   wordlist._array[spr_intScratche8]=temp;
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
int AlphabeticSorter::elemAt(char* stackStartPtr,SPRStackArray<int>& liste,int pos){
   char SPR_stack_dummy_var;
          int spr_intScratchea;spr_intScratchea=pos;chck_accs0(liste,spr_intScratchea,236)
   return liste._array[spr_intScratchea];
...
int AlphabeticSorter::isBelow(char* stackStartPtr,SPRStackArray<SPRSmartPtr<String_16> > liste,int pos,SPRSmartPtr<String_16> pivot){
   char SPR_stack_dummy_var;
          int spr_intScratchec;spr_intScratchec=pos;chck_accs0(liste,spr_intScratchec,238)
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   return this->isBelow(stackStartPtr,liste._array[spr_intScratchec],pivot);
...
int AlphabeticSorter::isAbove(char* stackStartPtr,SPRStackArray<SPRSmartPtr<String_16> > liste,int pos,SPRSmartPtr<String_16> pivot){
   char SPR_stack_dummy_var;
          int spr_intScratchee;spr_intScratchee=pos;chck_accs0(liste,spr_intScratchee,240)
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   return this->isBelow(stackStartPtr,pivot,liste._array[spr_intScratchee]);
...
int AlphabeticSorter::isAboveOrEqualPivot(char* stackStartPtr,SPRStackArray<SPRSmartPtr<String_16> > liste,int pos,SPRSmartPtr<String_16> pivot){
   ...
   SPRSmartPtr<String_16> l;
          int spr_intScratchf0;spr_intScratchf0=pos;chck_accs0(liste,spr_intScratchf0,242)
   l=liste._array[spr_intScratchf0];
   {//begin of SPR for statement
   int i;i=0;
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
void AlphabeticSorter::qsort(char* stackStartPtr,SPRStackArray<int>& liste,int lowerBound,int upperBound){
   ...
   int pivotIndex;pivotIndex=upperBound;
          int spr_intScratchf2;spr_intScratchf2=pivotIndex;chck_accs0(liste,spr_intScratchf2,244)
   int pivot;pivot=liste._array[spr_intScratchf2];
   int lower;lower=lowerBound;
   int upper;upper=upperBound-1;
   while(lower<upper){
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      ...
      if(lower<upper){
                int spr_intScratchf4;spr_intScratchf4=lower;chck_accs0(liste,spr_intScratchf4,246)
         int speicher;speicher=liste._array[spr_intScratchf4];
                int spr_intScratchf6;spr_intScratchf6=lower;chck_accs0(liste,spr_intScratchf6,248)
                int spr_intScratchf8;spr_intScratchf8=upper;chck_accs0(liste,spr_intScratchf8,250)
         liste._array[spr_intScratchf6]=liste._array[spr_intScratchf8];
                int spr_intScratchfa;spr_intScratchfa=upper;chck_accs0(liste,spr_intScratchfa,252)
         liste._array[spr_intScratchfa]=speicher;
         
      }
      
   };
          int spr_intScratchfc;spr_intScratchfc=upperBound;chck_accs0(liste,spr_intScratchfc,254)
          int spr_intScratchfe;spr_intScratchfe=lower;chck_accs0(liste,spr_intScratchfe,256)
   liste._array[spr_intScratchfc]=liste._array[spr_intScratchfe];
          int spr_intScratch100;spr_intScratch100=lower;chck_accs0(liste,spr_intScratch100,258)
   liste._array[spr_intScratch100]=pivot;
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
...
void AlphabeticSorter::test(char* stackStartPtr){
   ...
   SPRStackArrayConcrete<int,12> liste;
          int spr_intScratch102;spr_intScratch102=0;chck_accs0(liste,spr_intScratch102,260)
   liste._array[spr_intScratch102]=17;
          int spr_intScratch104;spr_intScratch104=1;chck_accs0(liste,spr_intScratch104,262)
   liste._array[spr_intScratch104]=33;
          int spr_intScratch106;spr_intScratch106=2;chck_accs0(liste,spr_intScratch106,264)
   liste._array[spr_intScratch106]=5;
          int spr_intScratch108;spr_intScratch108=3;chck_accs0(liste,spr_intScratch108,266)
   liste._array[spr_intScratch108]=8;
          int spr_intScratch10a;spr_intScratch10a=4;chck_accs0(liste,spr_intScratch10a,268)
   liste._array[spr_intScratch10a]=6;
          int spr_intScratch10c;spr_intScratch10c=5;chck_accs0(liste,spr_intScratch10c,270)
   liste._array[spr_intScratch10c]=7;
          int spr_intScratch10e;spr_intScratch10e=6;chck_accs0(liste,spr_intScratch10e,272)
   liste._array[spr_intScratch10e]=2;
          int spr_intScratch110;spr_intScratch110=7;chck_accs0(liste,spr_intScratch110,274)
   liste._array[spr_intScratch110]=99;
          int spr_intScratch112;spr_intScratch112=8;chck_accs0(liste,spr_intScratch112,276)
   liste._array[spr_intScratch112]=3088;
          int spr_intScratch114;spr_intScratch114=9;chck_accs0(liste,spr_intScratch114,278)
   liste._array[spr_intScratch114]=81;
          int spr_intScratch116;spr_intScratch116=10;chck_accs0(liste,spr_intScratch116,280)
   liste._array[spr_intScratch116]=92;
          int spr_intScratch118;spr_intScratch118=11;chck_accs0(liste,spr_intScratch118,282)
   liste._array[spr_intScratch118]=47;
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   this->qsort(stackStartPtr,liste,0,liste._sz-1);
   PrintfClass pfc0;
   SPRStackArrayConcrete<char,19> spr_StringScratch11a;strcpy(spr_StringScratch11a._array,"testing QUICKSORT");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   pfc0.fstr(stackStartPtr,spr_StringScratch11a).pr(stackStartPtr);
   ...
   while(i<wordlist._sz){
      ...
      SPRStackArrayConcrete<char,2> spr_StringScratch11d;strcpy(spr_StringScratch11d._array,"");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      wordlist._array[spr_intScratch11b]=::new String_16(stackStartPtr,spr_StringScratch11d);
      i++;
   ...
   SPRStackArrayConcrete<char,6> spr_StringScratch120;strcpy(spr_StringScratch120._array,"esel");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,14> spr_StringScratch123;strcpy(spr_StringScratch123._array,"eselsbruecke");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,6> spr_StringScratch126;strcpy(spr_StringScratch126._array,"zahn");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,13> spr_StringScratch129;strcpy(spr_StringScratch129._array,"zahnradbahn");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch12c;strcpy(spr_StringScratch12c._array,"apfel");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,9> spr_StringScratch12f;strcpy(spr_StringScratch12f._array,"annanas");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch132;strcpy(spr_StringScratch132._array,"saturn");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,6> spr_StringScratch135;strcpy(spr_StringScratch135._array,"mars");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch138;strcpy(spr_StringScratch138._array,"pluto");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch13b;strcpy(spr_StringScratch13b._array,"venus");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,5> spr_StringScratch13e;strcpy(spr_StringScratch13e._array,"son");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch141;strcpy(spr_StringScratch141._array,"father");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch144;strcpy(spr_StringScratch144._array,"mother");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,10> spr_StringScratch147;strcpy(spr_StringScratch147._array,"daughter");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch14a;strcpy(spr_StringScratch14a._array,"neues");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch14d;strcpy(spr_StringScratch14d._array,"leben");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch150;strcpy(spr_StringScratch150._array,"aussie");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch153;strcpy(spr_StringScratch153._array,"akzent");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,14> spr_StringScratch156;strcpy(spr_StringScratch156._array,"erforderlich");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,11> spr_StringScratch159;strcpy(spr_StringScratch159._array,"behaupten");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch15c;strcpy(spr_StringScratch15c._array,"trafen");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,7> spr_StringScratch15f;strcpy(spr_StringScratch15f._array,"kyjev");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch162;strcpy(spr_StringScratch162._array,"london");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch165;strcpy(spr_StringScratch165._array,"berlin");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,8> spr_StringScratch168;strcpy(spr_StringScratch168._array,"moscow");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   SPRStackArrayConcrete<char,10> spr_StringScratch16b;strcpy(spr_StringScratch16b._array,"chandler");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   ...
   while(i<wordlist._sz){
      ...
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      str.append(stackStartPtr,wordlist._array[spr_intScratch16c]);
      SPRStackArrayConcrete<char,5> spr_StringScratch16e;strcpy(spr_StringScratch16e._array,"[$]");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratch16e).sa(stackStartPtr,str).pr(stackStartPtr);
      i++;
...
void HexPrinter::intToHex(char* stackStartPtr,int x,SPRStackArray<char>& out,int startIndex){
   ...
   while(i<8){
      ...
      if(nibble<10){
                int spr_intScratch16f;spr_intScratch16f=i+startIndex;chck_accs0(out,spr_intScratch16f,369)
         out._array[spr_intScratch16f]=((char)nibble+'0');
         
      }
      else
      {
             int spr_intScratch171;spr_intScratch171=i+startIndex;chck_accs0(out,spr_intScratch171,371)
         out._array[spr_intScratch171]=((char)nibble+'a'-10);
         }
      i++;
...
void SystemControl::exitError(char* stackStartPtr,SPRStackArray<char>& message){
   char SPR_stack_dummy_var;
   PrintfClass pfc;
   SPRStackArrayConcrete<char,51> spr_StringScratch173;strcpy(spr_StringScratch173._array,"An Error occured because of '$'. Stopping program");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   pfc.fstr(stackStartPtr,spr_StringScratch173).sa(stackStartPtr,message).pr(stackStartPtr);
   ::exit(-1);
...
PrintfClass::PrintfClass(){
   ...
   char* stackStartPtr=list->_stackStartPtr;
   char SPR_stack_dummy_var;
   int spr_intScratch174;
   spr_intScratch174=32;
   if(spr_intScratch174<0)throw "invalid size requested for heap allocation";
   bufferDyn=new(spr_intScratch174) SPRArray<char>(spr_intScratch174)
   ;
...
void Benutzer_Umschalter::UnitTest(char* stackStartPtr){
   ...
   PrintfClass pfc;
   SPRStackArrayConcrete<char,37> spr_StringScratch175;strcpy(spr_StringScratch175._array,"bitte als Nutzer root laufen lassen");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   pfc.fstr(stackStartPtr,spr_StringScratch175).pr(stackStartPtr);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   bu.schalteNach(stackStartPtr,1000,1000);
   SPRStackArrayConcrete<char,13> spr_StringScratch176;strcpy(spr_StringScratch176._array,"/etc/shadow");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   SPRFile sf(stackStartPtr,spr_StringScratch176,1);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if(sf.systemHandle(stackStartPtr)>=0){
      SPRStackArrayConcrete<char,42> spr_StringScratch177;strcpy(spr_StringScratch177._array,"Benutzer_Umschalter::UnitTest() Fehler 1");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratch177).pr(stackStartPtr);
      
   }
   SPRStackArrayConcrete<char,35> spr_StringScratch178;strcpy(spr_StringScratch178._array,"nun sollen Fehlermeldungen kommen");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   pfc.fstr(stackStartPtr,spr_StringScratch178).pr(stackStartPtr);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   bu.schalteNach(stackStartPtr,0,0);
   SPRStackArrayConcrete<char,13> spr_StringScratch179;strcpy(spr_StringScratch179._array,"/etc/shadow");
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   SPRFile sf2(stackStartPtr,spr_StringScratch179,1);
   if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
   if(sf2.systemHandle(stackStartPtr)>=0){
      SPRStackArrayConcrete<char,42> spr_StringScratch17a;strcpy(spr_StringScratch17a._array,"Benutzer_Umschalter::UnitTest() Fehler 2");
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      if( MAX_STACK_SZ - (stackStartPtr-&SPR_stack_dummy_var) < __systemRuntime->_maxStackBytesNeededPerCall) overflowHandler();
      pfc.fstr(stackStartPtr,spr_StringScratch17a).pr(stackStartPtr);
